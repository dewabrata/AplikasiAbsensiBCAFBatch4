...
class CreditAdapter(val creditList: List<PengajuanCreditItem>) : RecyclerView.Adapter<CreditAdapter.CreditViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CreditViewHolder, position: Int) {
        ...
        Picasso.get().load(credit.fotoKtp).centerCrop().into(holder.imgKtpItem)
        Picasso.get().load(credit.fotoKk).centerCrop().into(holder.imgKKItem)
        Picasso.get().load(credit.fotoNpwp).centerCrop().into(holder.imgNPWPItem)
